...
INSERT INTO `inscripcion` (`idPersona`, `idEvento`, `tipoPago`, `monto`, `fecha`) VALUES
	...
	(3, 1, 'Gratis', 50.00, '2014-05-04 21:05:39'),
	(4, 1, 'Gratis', NULL, '2014-05-04 21:05:37'),
...
-- Volcando datos para la tabla db_eventos.persona: ~15 rows (aproximadamente)
...
BEGIN
		...
		WHERE e.tipo like concat(pbusqueda,'%');
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_S_Inscripcion`(
	pbusqueda VARCHAR(50)
	)
BEGIN
		SELECT * FROM inscripcion as i
		
		WHERE i.tipoPago like concat(pbusqueda,'%');
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_S_Persona`(
	pbusqueda VARCHAR(50)
	)
BEGIN
		SELECT * FROM Persona as p
		
		WHERE p.nombres like concat(pBusqueda,'%');
